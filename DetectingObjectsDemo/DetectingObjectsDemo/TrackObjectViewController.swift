...
class TrackObjectViewController: UIViewController {
    ...
    var observation = VNDetectedObjectObservation(boundingBox: CGRect(x: 0.3728713095188141, y: 0.833836019039154, width: 0.16493645310401917, height: 0.07572066783905029))

    lazy var request: VNTrackObjectRequest = {
        // 预测量完成的矩形区域
        
        let req = VNTrackObjectRequest(detectedObjectObservation: observation) { result, error in
            // 处理结果
            if let error {
//                print(error)
                
            }
            self.handleResult(request: result as! VNTrackObjectRequest)
        }
    ...
    func handleResult(request: VNTrackObjectRequest) {
        print(request.results)
        for r in request.results ?? [] {
            guard let result = r as? VNDetectedObjectObservation else {
                return
            }
            observation = result
